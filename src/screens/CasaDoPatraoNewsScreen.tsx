...
export function CasaDoPatraoNewsScreen() {
  ...
  const loadSaved = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch('/api/casa-do-patrao-news?read=1', {
        method: 'GET',
        cache: 'no-store',
      });
  ...
  const handleFetch = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch('/api/casa-do-patrao-news', {
        method: 'GET',
        cache: 'no-store',
      });
      const data = (await res.json()) as ApiResponse;
  ...
  const subtitle = useMemo(() => {
    ...
    if (loadingPhase === 'fetchRemote') return 'Buscando notícias na Record/R7...';
  ...
  const typeLabel = (t?: NewsItem['type']) => {
    if (!t) return null;
    return t === 'fotos' ? 'FOTOS' : 'NOVIDADE';
  };
