...
interface QueridometroExecution {
  startedAt: string;
  finishedAt?: string;
...
type QueridometroStatus = 'idle' | 'running' | 'success' | 'error';

export const useQueridometro = () => {
  const [status, setStatus] = useState<QueridometroStatus>('idle');
  const [lastExecution, setLastExecution] = useState<QueridometroExecution | null>(null);
  ...
  const runQueridometro = useCallback(async () => {
    ...
    console.log('🚀 Iniciando queridômetro - estado limpo');

    try {
      console.log('🔄 Iniciando conexão SSE do queridômetro...');
      const eventSource = new EventSource('/api/run-queridometro');
      ...
      eventSource.addEventListener('status', (e) => {
        ...
        console.log('🎯 SSE queridômetro status event received:', statusData);
      ...
      eventSource.onmessage = (event) => {
        console.log('📨 SSE queridômetro message:', event.data, 'type:', event.type);
      ...
      eventSource.onerror = (event) => {
        ...
        if (target && target.readyState === EventSource.CLOSED) {
          ...
          setTimeout(() => {
            ...
            if (!completedRef.current) {
              ...
              setLogs(prev => [...prev, '❌ Conexão perdida - verifique se o queridômetro terminou']);
              setStatus('error');
        ...
        console.error('❌ Erro crítico no EventSource do queridômetro:', event);
